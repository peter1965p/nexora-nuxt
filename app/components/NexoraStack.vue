...
<template>
  <section v-if="enabled" style="overflow:hidden;border-top:1px solid var(--nx-border)">
    ...
    <div style="border-top:1px solid var(--nx-border);padding:14px 0;overflow:hidden">
      <div class="ticker-track" style="display:flex;gap:10px;white-space:nowrap">
        <div v-for="r in 2" :key="r" style="display:flex;gap:10px;flex-shrink:0;align-items:center">
          <span v-for="item in items" :key="(item as any).label + r" :style="chipStyle((item as any).color)">
            {{ (item as any).label }}
          </span>
        </div>
      </div>
