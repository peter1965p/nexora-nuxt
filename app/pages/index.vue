  ...
  <div class="min-h-screen" style="background:var(--nx-bg);color:var(--nx-text);font-family:ui-monospace,SFMono-Regular,'SF Mono',Consolas,monospace">
    ...
    <footer class="w-full px-6 md:px-16 py-10 border-t" style="border-color:var(--nx-border)">
      <div class="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div v-if="tenant.pages?.length" class="flex items-center gap-4">
          <NuxtLink
            v-for="pg in tenant.pages"
            :key="pg.slug"
            :to="`/${pg.slug}`"
            class="text-[10px] uppercase tracking-widest transition-colors hover:opacity-100"
            style="color:var(--nx-muted);text-decoration:none"
          >
            {{ pg.title }}
          </NuxtLink>
        </div>
        <div class="flex items-center gap-2 text-[9px] uppercase tracking-widest" style="color:var(--nx-muted)">
